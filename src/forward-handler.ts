...
export class ForwardHandler {
  ...
  async handleUrlMessage(url: string, channelId: string): Promise<void> {
    try {
      ...
      await this.telegramClient.sendMessage(channelId, fallbackMessage);
    }
  }
  ...
  async handleForwardedMessage(
    ...
    try {
      ...
      await this.telegramClient.sendMessage(channelId, finalMessage);

    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      console.error('Failed to handle forwarded message:', errorMessage);

      await this.telegramClient.sendMessage(channelId, `⚠️ 转发失败: ${errorMessage}`);
    }
  }

  async handleOtherMessage(message: TelegramMessage, channelId: string): Promise<void> {
    try {
      ...
      if (message.text) {
        await this.telegramClient.sendMessage(channelId, message.text);
      }
    }
  }
}
